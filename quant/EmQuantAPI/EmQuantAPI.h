...
//用户可以自定义"ServerList.json"和"userInfo"文件的存放目录(如果不调用此函数或者dir传空则默认当前目录)
...
/**人工激活 适用于无界面运行环境（如远程linux）或无法运行LoginActivator程序的情况，激活成功后将获得的激活文件"userInfo"放到"ServerList.json"同级目录，再调用start登录
*  参数说明：
