...
int KafkaProducer::Init(std::string brokers, 
                        std::string topic, 
                        std::string group) {
  char errstr[512];
  ...
  if (RD_KAFKA_CONF_OK != 
      rd_kafka_conf_set(conf, "group.id", group.c_str(), NULL, 0)) {
    return -1;
  }
  ...
  /* Create topic */
  kafka_control_.topic = rd_kafka_topic_new(kafka_control_.control,
                                            topic.c_str(), 
                                            topic_conf);
